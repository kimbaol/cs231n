...
def rnn_backward(dh, cache):
  ...
  for t in xrange(T-1,-1,-1):
    if t == 0:
      cache = x[:,t,:],h0,Wx,Wh,h[:,t,:]
    else:
      cache = x[:,t,:],h[:,t-1,:],Wx,Wh,h[:,t,:]
    dx[:,t,:],dh_prev,dWx_t,dWh_t,db_t = rnn_step_backward(dh[:,t,:]+dh_prev,cache)
    dWx += dWx_t
    dWh += dWh_t
    db += db_t
  dh0 = dh_prev
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  hi, hf, ho, hg = np.split(h, 4 , axis=1)
  i,f,o,g = sigmoid(hi), sigmoid(hf), sigmoid(ho), np.tanh(hg)
  next_c = f*prev_c + i*g
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  dnext_c += (1-tanh_next_c**2)*o*dnext_h
  df = dnext_c*prev_c
  dprev_c = dnext_c*f
  di = dnext_c*g
  dg = dnext_c*i
  ...
  dho = do*o*(1-o)
  dhg = dg*(1-g**2)
  dh = np.concatenate((dhi,dhf,dho,dhg),axis=1)
  dx = dh.dot(Wx.T)
  dWx = x.T.dot(dh)
  dprev_h = dh.dot(Wh.T)
  dWh = prev_h.T.dot(dh)
...
def lstm_backward(dh, cache):
  ...
  for t in xrange(T-1,-1,-1):
    dx[:,t,:],dh_prev,dc_prev,dWx_t,dWh_t,db_t = lstm_step_backward(dh[:,t,:]+dh_prev,dc_prev,caches.pop())
    dWx += dWx_t
    dWh += dWh_t
    db += db_t
  dh0 = dh_prev
